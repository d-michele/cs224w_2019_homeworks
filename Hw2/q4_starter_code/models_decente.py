...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def update(self, aggr_out, self_feat):
        ...
        # print(f'aggr {aggr_out.detach().cpu().numpy()[0]}')
        # print(f'node_feat{node_feat.detach().cpu().numpy()[0]}')
        aggr_out = self.lin(torch.cat([self_feat, aggr_out], axis=1))
        # print(aggr_out.size())
        # print(node_feat.size())
        # aggr_out = F.relu(aggr_out)
        # aggr_out = torch.cat([self_feat, aggr_out], dim=1)
        # aggr_out = torch.cat([self_feat, aggr_out], dim=1)

        if self.normalize_emb:
            # aggr_out = node_feat + self.agg_lin(aggr_out)
            aggr_out = F.normalize(aggr_out, p=2., dim=1)

            # aggr_out = aggr_out / torch.linalg.norm()
        ############################################################################
